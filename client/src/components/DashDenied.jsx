...
const DashDenied = () => {
  ...
  const deniedAssignments = userAssignments.filter(
    (assignment) => assignment.isDenied
  );
      ...
      try {
        ...
        if (res.ok) {
          const filteredAssignments = data.assignments.filter(
            (assignment) =>
              assignment.isDenied && assignment.userId._id === currentUser._id
          );
  ...
  return (
    <div className="overflow-x-auto p-2 w-full ">
      {!currentUser.isAdmin && deniedAssignments.length > 0 ? (
        <>
          <Table striped className="border">
            ...
            {deniedAssignments.map((assignment) => (
